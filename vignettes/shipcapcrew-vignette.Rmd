...
*Each of these helper functions listed direct below checks for a plethora of arguments:*
...
`rolling_function`: A function that simulates the rolling of a size-sided die, with each side having an equal probability of facing up. It determines whether dice values can be kept in player's hand, or have to be returned to be rerolled. It performs similarly to the `reroll` and `check reroll` functions in the *Interactive Gameplay* family.

`strategy_function`: Determines which, if any, strategy a player is using for a game simulation, and it calls on the appropriate function that runs a specific strategy.

`greedy_reroll`: Is a function that contains a strategy where the player rerolls each time it is made available to them, regardless of their current score or number of rolls remaining.

`final_scores_tab`: Which calculates a players current score, after they have either collected after all the required dice are obtained or the player runs out of rolls. It performs similar to the `find_score` function in the *Interactive Gameplay* family.

`add_scores`: Creates a data frame and appends latest player’s score, and presents this information to the user as a as a gamecard.

None of these helper functions explicitly return anything. Together, these family of functions exist to help the main family functions create the user-facing simulated gameplay.

## **Simulation Functions Family**

There are three functions that make up this second family that the user can interact with will working with the *Simulation Functions* collection of functions: `start_simulation()`, `multiplayer_simulation()`, and `reset_simulation()`.

### `start_simulation()`

This function initializes a single-player game. It reads in the number of games the user would like to be simulated, the strategy the player would like the simulation to optimize, and indicates that the gameplay is single player. It returns a data frame containing the 
...
This function removes the previous multiplayer gamecards from environment, so that the user can run new simulation. Users should be advised to reset the simulations after running each multiplayer gameplay simulation.
